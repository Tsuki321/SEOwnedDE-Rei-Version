...
namespace {
constexpr const char* kFeatureDir = "SEOwnedDE/SEOwnedDE/src/App/Features/MiscVisuals";
constexpr const char* kMainSource = "SEOwnedDE/SEOwnedDE/src/App/Features/MiscVisuals/MiscVisuals.cpp";
}

TEST(MiscVisualsContracts, ContainsExpectedSourceFiles) {
...
TEST(MiscVisualsContracts, MainSourceContainsFeatureTokens) {
    const auto root = testhelpers::FindRepoRoot();
    ...
    EXPECT_NE(mainSource.find("CFG::Visuals_Aimbot_FOV_Circle"), std::string::npos);
    EXPECT_NE(mainSource.find("H::Entities"), std::string::npos);
    EXPECT_NE(mainSource.find("CMiscVisuals::AimbotFOVCircle("), std::string::npos);
    EXPECT_GE(testhelpers::CountTokenAcrossFiles(cppFiles, "CFG::"), 1u);
}

TEST(MiscVisualsContracts, UsesGuardClausesAndReturns) {
    const auto root = testhelpers::FindRepoRoot();
    const auto cppFiles = testhelpers::CollectFiles(root / kFeatureDir, ".cpp");
