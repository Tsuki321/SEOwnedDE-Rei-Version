...
namespace {
constexpr const char* kFeatureDir = "SEOwnedDE/SEOwnedDE/src/App/Features/TeamWellBeing";
constexpr const char* kMainSource = "SEOwnedDE/SEOwnedDE/src/App/Features/TeamWellBeing/TeamWellBeing.cpp";
}

TEST(TeamWellBeingContracts, ContainsExpectedSourceFiles) {
...
TEST(TeamWellBeingContracts, MainSourceContainsFeatureTokens) {
    const auto root = testhelpers::FindRepoRoot();
    ...
    EXPECT_NE(mainSource.find("CFG::Visuals_TeamWellBeing_Pos_X"), std::string::npos);
    EXPECT_NE(mainSource.find("H::Input"), std::string::npos);
    EXPECT_NE(mainSource.find("CTeamWellBeing::Drag("), std::string::npos);
    EXPECT_GE(testhelpers::CountTokenAcrossFiles(cppFiles, "CFG::"), 1u);
}

TEST(TeamWellBeingContracts, UsesGuardClausesAndReturns) {
    const auto root = testhelpers::FindRepoRoot();
    const auto cppFiles = testhelpers::CollectFiles(root / kFeatureDir, ".cpp");
