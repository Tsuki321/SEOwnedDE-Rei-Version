...
namespace {
constexpr const char* kFeatureDir = "SEOwnedDE/SEOwnedDE/src/App/Features/WorldModulation";
constexpr const char* kMainSource = "SEOwnedDE/SEOwnedDE/src/App/Features/WorldModulation/WorldModulation.cpp";
}

TEST(WorldModulationContracts, ContainsExpectedSourceFiles) {
...
TEST(WorldModulationContracts, MainSourceContainsFeatureTokens) {
    const auto root = testhelpers::FindRepoRoot();
    ...
    EXPECT_NE(mainSource.find("CFG::Visuals_World_Modulation_Mode"), std::string::npos);
    EXPECT_NE(mainSource.find("CWorldModulation::ApplyModulation("), std::string::npos);
    EXPECT_GE(testhelpers::CountTokenAcrossFiles(cppFiles, "CFG::"), 1u);
}

TEST(WorldModulationContracts, UsesGuardClausesAndReturns) {
    const auto root = testhelpers::FindRepoRoot();
    const auto cppFiles = testhelpers::CollectFiles(root / kFeatureDir, ".cpp");
