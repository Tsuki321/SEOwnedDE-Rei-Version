...
namespace {
constexpr const char* kFeatureDir = "SEOwnedDE/SEOwnedDE/src/App/Features/SeedPred";
constexpr const char* kMainSource = "SEOwnedDE/SEOwnedDE/src/App/Features/SeedPred/SeedPred.cpp";
}

TEST(SeedPredContracts, ContainsExpectedSourceFiles) {
...
TEST(SeedPredContracts, MainSourceContainsFeatureTokens) {
    const auto root = testhelpers::FindRepoRoot();
...
TEST(SeedPredContracts, UsesGuardClausesAndReturns) {
    const auto root = testhelpers::FindRepoRoot();
    const auto cppFiles = testhelpers::CollectFiles(root / kFeatureDir, ".cpp");
