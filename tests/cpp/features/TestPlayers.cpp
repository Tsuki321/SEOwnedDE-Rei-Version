...
namespace {
constexpr const char* kFeatureDir = "SEOwnedDE/SEOwnedDE/src/App/Features/Players";
constexpr const char* kMainSource = "SEOwnedDE/SEOwnedDE/src/App/Features/Players/Players.cpp";
}

TEST(PlayersContracts, ContainsExpectedSourceFiles) {
...
TEST(PlayersContracts, MainSourceContainsFeatureTokens) {
    const auto root = testhelpers::FindRepoRoot();
    ...
    EXPECT_NE(mainSource.find("CPlayers::Parse("), std::string::npos);
}

TEST(PlayersContracts, UsesGuardClausesAndReturns) {
    const auto root = testhelpers::FindRepoRoot();
    const auto cppFiles = testhelpers::CollectFiles(root / kFeatureDir, ".cpp");
