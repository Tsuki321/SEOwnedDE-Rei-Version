...
namespace {
constexpr const char* kFeatureDir = "SEOwnedDE/SEOwnedDE/src/App/Features/Chat";
constexpr const char* kMainSource = "SEOwnedDE/SEOwnedDE/src/App/Features/Chat/Chat.cpp";
}

TEST(ChatContracts, ContainsExpectedSourceFiles) {
    ...
    EXPECT_GE(headerFiles.size(), static_cast<std::size_t>(0));
}

TEST(ChatContracts, MainSourceContainsFeatureTokens) {
    const auto root = testhelpers::FindRepoRoot();
    ...
    EXPECT_NE(mainSource.find("CFG::Misc_Chat_Owoify"), std::string::npos);
    EXPECT_NE(mainSource.find("std::regex_replace("), std::string::npos);
    EXPECT_GE(testhelpers::CountTokenAcrossFiles(cppFiles, "CFG::"), 1u);
}

TEST(ChatContracts, UsesGuardClausesAndReturns) {
    const auto root = testhelpers::FindRepoRoot();
    const auto cppFiles = testhelpers::CollectFiles(root / kFeatureDir, ".cpp");
