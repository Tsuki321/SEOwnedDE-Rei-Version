...
namespace {
constexpr const char* kFeatureDir = "SEOwnedDE/SEOwnedDE/src/App/Features/SpectatorList";
constexpr const char* kMainSource = "SEOwnedDE/SEOwnedDE/src/App/Features/SpectatorList/SpectatorList.cpp";
}

TEST(SpectatorListContracts, ContainsExpectedSourceFiles) {
...
TEST(SpectatorListContracts, MainSourceContainsFeatureTokens) {
    const auto root = testhelpers::FindRepoRoot();
    ...
    EXPECT_NE(mainSource.find("CFG::Visuals_SpectatorList_Width"), std::string::npos);
    EXPECT_NE(mainSource.find("H::Entities"), std::string::npos);
    EXPECT_NE(mainSource.find("CSpectatorList::GetSpectators("), std::string::npos);
    EXPECT_GE(testhelpers::CountTokenAcrossFiles(cppFiles, "CFG::"), 1u);
}

TEST(SpectatorListContracts, UsesGuardClausesAndReturns) {
    const auto root = testhelpers::FindRepoRoot();
    const auto cppFiles = testhelpers::CollectFiles(root / kFeatureDir, ".cpp");
